$(document).ready(function() {
  var tronaldDump = [
    'PANIC SELLING!!!',
    'DUMPPP!',
    'Bitcoin going to 0!! LOLOLOL',
    'SEGWIT POISON PILL',
    'LIGHTNING VAPORWARE',
    'buy da dip',
    'Bitcoin up to 10% off',
    'Tronald DUMP!',
    "RIP McAfee's dick"
  ];
  var hodlersBelike = [
    ...
    'SELL WIFE! BUY BITCOIN!!',
    'buckle up hodlers',
    "can't see any altcoin up here",
    'hey r/bitcoin, should I buy today?'
  ];
  var meh = [
    'meh..',
    'mmm...',
  ...
  function fetchMooningPrices() {
    ...
    for (i = 0; i < markets.length; i++) {
      if (i != selectedMarketIndex) markets[i].fetchPrices(updateTicker);
    }
  }
  ...
  function updateCurrentMoon(open, close) {
    if (currentMoon != close) {
      ...
      $('#change-percentage').html(
        signal + Math.abs((currentMoon / oldEarth - 1) * 100).toFixed(2) + '%'
      );
  ...
  function updateStatus(open, close) {
    ...
    if (close >= 100000) {
      ...
      $('body').css(
        'background-image',
        'url(images/what-if-its-a-space-rollercoaster.jpg'
      );
    }
    ...
    if (changeAbs >= changeTreshold) {
      ...
      $('#roller-coaster-guy').attr(
        'src',
        'images/no-fun-roller-coaster-guy.gif'
      );
      rotateTheGuy(-angle);
    }
  ...
  function makeFeeGreatAgain(data) {
    ...
    $('#fastest-avg-fee-fiat').html(
      '$ ' + Number(fastestAvgFeePerTx).toFixed(3)
    );
  }
